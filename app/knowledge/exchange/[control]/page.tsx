...
const controls: Record<string, { id: string; title: string; category: string; aliases: string }> = {
  'exo-mf-001': { id: 'EXO-MF-001', title: 'Accepted Domains', category: 'Mail Flow', aliases: 'accepted domains and domain routing configuration' },
  'exo-mf-002': { id: 'EXO-MF-002', title: 'Connectors', category: 'Mail Flow', aliases: 'Exchange Online mail flow connectors and transport routing' },
  'exo-mf-004': { id: 'EXO-MF-004', title: 'DMARC', category: 'Mail Authentication', aliases: 'DMARC, domain authentication policy, and alignment' },
  'exo-mf-005': { id: 'EXO-MF-005', title: 'Remote Domains', category: 'Mail Flow', aliases: 'remote domain configuration and external mail behavior' },
  'exo-sec-001': { id: 'EXO-SEC-001', title: 'Anti-Spam Policies', category: 'Security', aliases: 'anti-spam policy configuration and messaging protection' },
  'exo-sec-002': { id: 'EXO-SEC-002', title: 'Authentication Policies', category: 'Authentication', aliases: 'Exchange authentication policies and legacy authentication controls' },
  'exo-sec-003': { id: 'EXO-SEC-003', title: 'External Forwarding', category: 'Security', aliases: 'external automatic forwarding and outbound message controls' },
  'exo-sec-004': { id: 'EXO-SEC-004', title: 'Mailbox Auditing', category: 'Security', aliases: 'mailbox audit configuration and audit coverage' },
  'exo-sec-005': { id: 'EXO-SEC-005', title: 'SMTP AUTH', category: 'Authentication', aliases: 'SMTP AUTH configuration and authenticated SMTP exposure' },
};

const panel = { border: '1px solid rgba(86,160,255,.2)', borderRadius: 16, background: 'rgba(8,22,40,.68)', padding: 22 } as const;

export default async function ExchangeKnowledgeArticle({ params }: { params: Promise<{ control: string }> }) {
    ...
    <main style={{ minHeight: '100vh', background: 'linear-gradient(180deg,#07111f 0%,#0d1321 100%)', color: '#f3f6fb' }}>
      ...
      <div style={{ width: 'min(980px,100%)', margin: '0 auto', padding: '38px 20px 72px' }}>
        ...
        <header style={{ margin: '24px 0 28px' }}>
          ...
          <p style={{ margin: 0, color: '#aeb8c8', fontSize: 17, lineHeight: 1.65, maxWidth: 820 }}>TenantIQ uses {article.id} to assess {article.aliases} in Exchange Online. Guidance remains evidence-bound so TenantIQ does not invent tenant configuration that was not observed.</p>
        </header>
        ...
        <div style={{ display: 'grid', gap: 16 }}>
          <section style={panel}>
            ...
            <p style={{ margin: 0, color: '#b4c1cf', lineHeight: 1.7 }}>This check evaluates the TenantIQ control represented by {article.title}. A FAIL or WARNING means the observed configuration, coverage, inventory, or governance state requires review against the organization&apos;s approved Microsoft 365 baseline.</p>
          </section>
          ...
          <section style={panel}>
            ...
            <ul style={{ margin: 0, paddingLeft: 20, color: '#b4c1cf', lineHeight: 1.85 }}>
              ...
              <li>Counts, configuration values, objects, identities, domains, or scope explicitly present in that evidence.</li>
          ...
          <section style={panel}>
            ...
            <p style={{ margin: 0, color: '#b4c1cf', lineHeight: 1.7 }}>Weak or incomplete configuration in this area can create avoidable security, compliance, governance, reliability, or operational risk. TenantIQ should tie risk statements directly to the finding&apos;s status, severity, evidence, and recommendation.</p>
          </section>
          ...
          <section style={panel}>
            ...
            <ol style={{ margin: 0, paddingLeft: 22, color: '#b4c1cf', lineHeight: 1.85 }}>
              ...
              <li>Re-run the TenantIQ assessment and verify the finding state changed as expected.</li>
            </ol>
          </section>
