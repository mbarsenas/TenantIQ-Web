  ...
  return <main style={{minHeight:'100vh',background:'linear-gradient(180deg,#07111f 0%,#0d1321 100%)',color:'#f3f6fb'}}>
    ...
    <div style={{width:'min(1100px,100%)',margin:'0 auto',padding:'38px 20px 72px'}}>
      ...
      <section style={{border:'1px solid rgba(86,160,255,.2)',borderRadius:16,background:'rgba(8,22,40,.68)',padding:22}}>
        ...
        <p style={{margin:'0 0 20px',color:'#9fb0c2',lineHeight:1.6,fontSize:14,maxWidth:800}}>Start with the control areas currently represented in the TenantIQ knowledge repository. Access Reviews now has a full knowledge article; the remaining controls can still be sent directly to the Knowledge Assistant.</p>
        <div style={{display:'grid',gap:0}}>{controls.map(([id,title,category,description])=>{const hasArticle=id==='ENTRA-GOV-001';return <div key={id} style={{display:'grid',gridTemplateColumns:'minmax(120px,.8fr) minmax(180px,1.3fr) minmax(140px,1fr) minmax(220px,1.8fr) auto',gap:14,alignItems:'center',borderTop:'1px solid rgba(86,160,255,.13)',padding:'14px 4px'}}><div style={{color:'#79baff',fontSize:12,fontWeight:900}}>{id}</div><div style={{fontSize:14,fontWeight:800}}>{title}</div><div style={{color:'#8fa4ba',fontSize:12}}>{category}</div><div style={{color:'#9fb0c2',fontSize:12,lineHeight:1.45}}>{description}</div><a href={hasArticle?'/knowledge/entra/entra-gov-001':`/assistant?finding=${encodeURIComponent(id)}`} style={{color:'#78b8ff',fontSize:12,fontWeight:850,textDecoration:'none',whiteSpace:'nowrap'}}>{hasArticle?'Read article →':'Ask Assistant →'}</a></div>})}</div>
      </section>
